...
guiado**. Un Web Component (`<closer-click-tutorial>`, Shadow DOM) reutilizable
por cualquier app (Vue o vanilla): muestra **burbujas iguales a la de
donar/compartir** (`<closer-click-support>`), pero ancladas a elementos reales de
...
```bash
npm i @closerclick/closer-click-tutorial
```
...
`<closer-click-tutorial>`, que **es** el controlador.
...
```html
<closer-click-tutorial lang="es" storage-key="miapp.tutorial"></closer-click-tutorial>
```
...
```css
closer-click-tutorial::part(next) { background: #fff; color: #111; }
closer-click-tutorial::part(bubble) { font-weight: 600; }
```
...
Parte del ecosistema [Closer Click](https://closer.click): *tu información, en tu
